...
function sub_form(id) {
    // la.l.fadeIn()
    $.ajax({
        url: 'api.php?do=assign_sub_form',
        type: 'post',
        data: {
            user_serial: id
        },
...
function case_get() {
    la.l.fadeIn();
    $.ajax({
        url: 'api.php?do=assign_case_get',
        type: 'post',
        data: {
...
function go_back() {
    ...
    f.serial_back.value = order_serial;
    f.submit();
}
</script>
